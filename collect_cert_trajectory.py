# collect_cert_trajectory.py
# 충남대 증명서 발급 AI Agent trajectory 수집 스크립트
# collect_trajectory.py (도서관) 구조 기반, 증명서 발급 특화 버전
...
async def run_task(task_info: dict, task_index: int, output_dir: Path, data_dir: Path) -> dict:
    ...
    full_task = f"{CERT_BASE_CONTEXT} {raw_task}" if use_base_context else raw_task

    print(f"\n{'='*60}")
    print(f"[{task_index+1}/{len(ALL_TASKS)}] [{category}] {raw_task[:50]}...")
    print('='*60)

    allowed_domains = CATEGORY_DOMAINS.get(category, ["cnu.icerti.com"])
    ...
    agent = LoggedAgent(
        ...
        save_conversation_path=str(data_dir / "conversations" / f"cert_task_{task_index:03d}.json"),
        generate_gif=str(data_dir / "gifs" / f"cert_task_{task_index:03d}.gif"),
    ...
    try:
        ...
        output_path = output_dir / f"cert_task_{task_index:03d}.json"
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(trajectory, f, ensure_ascii=False, indent=2)

        # trajectory JSONL 저장 (전체 누적)
        ...
        error_path = output_dir / f"cert_task_{task_index:03d}_error.json"
...
async def main():
    # 디렉토리 생성
    output_dir = Path("cert_trajectories")
    data_dir = Path("data")
    ...
    # 실행 범위 설정
    # 방법1) 환경변수: START_TASK=0 END_TASK=10 python collect_cert_trajectory.py
    # 방법2) 커맨드라인: python collect_cert_trajectory.py 0 10
    ...
    for i in range(start, end):
        ...
        if len(all_results) % 10 == 0:
            ...
            with open(data_dir / "results" / f"cert_progress_{timestamp}.json", "w", encoding="utf-8") as f:
    ...
    # 최종 저장
    with open(data_dir / "results" / "cert_final_results.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)

    print(f"\n결과 저장: data/results/cert_final_results.json")
    print(f"trajectory: cert_trajectories/")
