# collect_all_vl.py
# collect_all.py 기반, LoggedAgentVL 사용 (스크린샷 포함 저장)
# 사용법:
#   python collect_all_vl.py --model bu 0 100
#   python collect_all_vl.py --model gpt 0 100
#   python collect_all_vl.py --model gemini 0 100
#   python collect_all_vl.py --model gpt --cert 0 100
...
def get_dirs(model_name: str, is_cert: bool) -> dict:
    ...
    return {
        "output_dir": Path(f"{cert_prefix}trajectories_vl{model_suffix}"),
        "data_dir": Path("data"),
        "training_data": Path(f"data/training_data_vl{model_suffix}.jsonl"),
        "conversations": Path(f"data/conversations_vl{model_suffix}"),
        "gifs": Path(f"data/gifs_vl{model_suffix}"),
...
async def run_task(
    ...
    try:
        ...
        jsonl_path = dirs["data_dir"] / f"trajectories_vl_{model_name}.jsonl"
...
async def main():
    ...
    for i in range(start, end):
        ...
        if len(all_results) % 10 == 0:
            ...
            with open(dirs["results"] / f"{cert_prefix}progress_vl_{args.model}_{timestamp}.json", "w", encoding="utf-8") as f:
    ...
    cert_prefix = "cert_" if args.cert else ""
    with open(dirs["results"] / f"{cert_prefix}final_vl_{args.model}.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)

    print(f"\n결과 저장: data/results/{cert_prefix}final_vl_{args.model}.json")
